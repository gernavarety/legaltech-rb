...
AI-генератор составляет готовые юридические документы по праву РБ на основе введённых данных.
Работает через Claude API, форматирует в DOCX, конвертирует в PDF через LibreOffice.

### 20 шаблонов в 4 группах

| Группа | Шаблоны |
|--------|---------|
| Договоры | Купли-продажи, аренды помещения, аренды ТС, поставки, подряда, услуг, займа, хранения, комиссии, перевозки |
| Трудовые | Трудовой контракт, доп. соглашение, материальная ответственность, должностная инструкция |
| Корпоративные | Устав ООО, протокол собрания, решение единственного участника, доверенность |
| Претензии | Претензионное письмо, ответ на претензию |

### Лимиты по тарифам

| | FREE | SOLO | FIRM |
|--|------|------|------|
| Генераций/мес | 3 | 30 | Безлимит |
| Скачать DOCX | ✓ | ✓ | ✓ |
| Скачать PDF | — | ✓ | ✓ |
| Приоритет в очереди | — | — | ✓ |
...
```bash
# 1. Применить новые миграции (007, 008, 009)
psql $DATABASE_URL -f migrations/000_run_all.sql

# 2. Загрузить 20 шаблонов в БД
docker compose exec backend python scripts/seed_templates.py

# 3. Запустить worker с LibreOffice (для PDF конвертации)
docker compose up -d worker-generation
...
Пользователь заполняет форму
         ↓
POST /api/generate → Celery (очередь generation/priority)
         ↓
generation_task.py:
  1. Поиск норм РБ в pgvector
  2. Формирование промпта (generation_prompts.py)
  3. Claude API → текст документа
  4. python-docx → DOCX (utils/docx_builder.py)
  5. LibreOffice headless → PDF (utils/pdf_converter.py)
  6. Cloudflare R2 → хранение файлов
         ↓
GET /api/generate/{task_id} (polling каждые 2.5 сек)
         ↓
Страница результата: превью + кнопки скачивания
```

### Docker worker для LibreOffice

Генерация PDF требует LibreOffice. Для этого используется отдельный Dockerfile:

```bash
# Сборка тяжёлого image (~800 МБ из-за LibreOffice)
...
Без `worker-generation` документы генерируются только в DOCX формате.
